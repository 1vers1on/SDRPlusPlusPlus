...
namespace backend {
    ...
    int init(std::string resDir) {
        ...
        for (int i = 0; i < OPENGL_VERSION_COUNT; i++) {
            ...
            glfwWindowHintString(GLFW_WAYLAND_APP_ID, "sdrppp");
    #endif
        ...
        // Load app icon
        if (!std::filesystem::is_regular_file(resDir + "/icons/sdrppp.png")) {
            flog::error("Icon file '{0}' doesn't exist!", resDir + "/icons/sdrppp.png");
            return 1;
        }

        GLFWimage icons[10];
        icons[0].pixels = stbi_load(((std::string)(resDir + "/icons/sdrppp.png")).c_str(), &icons[0].width, &icons[0].height, 0, 4);
